...
import { getFloor, stairEscalatorCount, lobbyCovers, floorContains } from './tower';
...
export function elevatorPlacementError(
  ...
  for (let f = floorLo; f <= floorHi; f++) {
    ...
    if (f !== CONFIG.LOBBY_FLOOR_INDEX && !floorContains(state, f, x, width)) return 'Extend this floor first';
...
export function escalatorPlacementError(
  ...
  for (const floor of [lower, upper]) {
    if (floor.index !== CONFIG.LOBBY_FLOOR_INDEX && !floorContains(state, floor.index, x, 8)) return 'Extend this floor first';
